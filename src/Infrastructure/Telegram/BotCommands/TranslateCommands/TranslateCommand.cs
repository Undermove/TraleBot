...
public class TranslateCommand(ITelegramBotClient client, IMediator mediator) : IBotCommand
{
    ...
    public async Task Execute(TelegramRequest request, CancellationToken token)
    {
        ...
        await (result switch
        {
            CreateVocabularyEntryResult.TranslationSuccess success => client.SendTranslation(request, success.VocabularyEntryId, success.Definition, success.AdditionalInfo, success.Example, token),
            CreateVocabularyEntryResult.TranslationExists exists => client.SendExistedTranslation(request, exists.VocabularyEntryId, exists.Definition, exists.AdditionalInfo, exists.Example, token),
