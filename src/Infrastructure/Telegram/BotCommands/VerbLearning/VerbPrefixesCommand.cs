...
public class VerbPrefixesCommand(IMediator mediator, ITelegramBotClient client)
    : IBotCommand
{
    public Task<bool> IsApplicable(TelegramRequest request, CancellationToken ct)
    {
        ...
        return Task.FromResult(
            text.Equals(CommandNames.VerbPrefixes, StringComparison.InvariantCultureIgnoreCase) ||
            text.StartsWith(CommandNames.VerbPrefixesIcon, StringComparison.InvariantCultureIgnoreCase));
    }
    ...
    public async Task Execute(TelegramRequest request, CancellationToken token)
    {
        ...
        if (cardResult is GetNextVerbCardResult.NoCardsAvailable)
        {
            await client.SendTextMessageAsync(
                ...
                "🎉 Ты прошёл все приставки! Приходи позже для повторения.",
