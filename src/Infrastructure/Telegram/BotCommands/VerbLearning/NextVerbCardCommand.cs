...
public class NextVerbCardCommand(IMediator mediator, ITelegramBotClient client)
    : IBotCommand
{
    public Task<bool> IsApplicable(TelegramRequest request, CancellationToken ct)
    {
        ...
        return Task.FromResult(
            text.Equals(CommandNames.NextVerbCard, StringComparison.InvariantCultureIgnoreCase) ||
            text.StartsWith(CommandNames.NextVerbCardIcon, StringComparison.InvariantCultureIgnoreCase));
    }
    ...
    public async Task Execute(TelegramRequest request, CancellationToken token)
    {
        ...
        var query = new GetNextVerbCardQuery { UserId = request.User.Id };
        var result = await mediator.Send(query, token);

        if (result is GetNextVerbCardResult.NoCardsAvailable)
        {
        ...
        if (result is not GetNextVerbCardResult.CardReady ready)
            return;
